"""
Downloader for creators.com comics:
  - Wizard of Id  (id: wiz)

When this breaks:
  - Check last_known_good_cache/wiz.html vs the current page source
  - Look for <meta property="og:image" content="..."> — that Open Graph tag
    is the most stable way to find the comic image URL on creators.com
"""
...
async def download_creators(
    ...
    **kwargs,
    ...
    try:
        ...
        if not og_image:
            raise RuntimeError("No <meta property='og:image'> found on page")

        img_url = og_image.get("content", "")
        if not img_url or not img_url.startswith("http"):
            raise RuntimeError(f"og:image content is missing or not a URL: {img_url!r}")
